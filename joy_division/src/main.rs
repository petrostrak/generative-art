...
fn view(app: &App, _model: &Model, frame: Frame) {
    ...
    // Attempt to implement bezier curve formula
    // for i in 0..lines.len() - 5 {
    //     for j in 0..lines[i].len() - 1 {
    //         let xc = (lines[i][j].x + lines[i][j + 1].x) / 2.0;
    //         let yc = (lines[i][j].y + lines[i][j + 1].y) / 2.0;

    //         draw.path().stroke().weight(LINE_WIDTH).color(BLACK).events(
    //             nannou::geom::Path::builder()
    //                 .begin(pt2(lines[i][0].x, lines[i][0].y))
    //                 .quadratic_bezier_to(pt2(lines[i][j].x, lines[i][j].y), pt2(xc, yc))
    //                 .build()
    //                 .iter(),
    //         );
    //     }
