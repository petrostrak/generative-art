...
impl Mondrian {
    pub fn generate(&mut self, nest: u32) -> Document {
        let mut v = vec![Rectangle {
            x1: 0.0,
            y1: 0.0,
            x2: self.width * self.size,
            y2: 1.0 * self.size,
        }];
        for _ in 0..nest {
            v = self.split(v)
    ...
    pub fn rectangle_svg(&mut self, input: Vec<Rectangle>) -> Document {
        let mut document =
            Document::new().set("viewBox", (0, 0, self.width * self.size, 1.0 * self.size));
        for Rectangle { x1, y1, x2, y2 } in input {
    ...
    fn svg_line(&self, x1: f32, y1: f32, x2: f32, y2: f32) -> svg::node::element::Line {
        svg::node::element::Line::new()
            ...
            .set(
                "style",
                format!("stroke:black;stroke-width:{}", self.line_width),
            )
    }
